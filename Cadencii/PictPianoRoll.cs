        ...
        public void paint(Graphics g1)
        {
            ...
            lock (AppManager.mDrawObjects) {
                ...
                try {
                    ...
                    RendererKind renderer = RendererKind.UTAU;
/*                    if (vsq != null) {
                            ...
                            if (dobj.mType == DrawObjectType.Note) {
                                ...
                                if (dobj.mIsOverlapped) {
                                    ...
                                    if (AppManager.editorConfig.ViewAtcualPitch || AppManager.mCurveOnPianoroll) {
                                        ...
                                        int ItemID = vsq_track.findEventIndexFromID(dobj.mInternalID);
                                        VsqEvent item_prev = null;
                                        VsqEvent item_next = null;
                                        VsqEvent item = null;
                                        try
                                        {
                                            item = vsq_track.getEvent(ItemID);
                                            try
                                            {
                                                item_prev = vsq_track.getEvent(ItemID - 1);
                                            }
                                            catch { ;}
                                            try
                                            {
                                                item_next = vsq_track.getEvent(ItemID + 1);
                                            }
                                            catch { ;}
                                        }
                                        catch { ;}
                                        cadencii.dsp.generator.UtauMath.interfaceNoteLine LineK = null;
                                        ...
                                        VsqEvent checkEvent = item;

                                        int FS = 0;
                                        int XLengthHead = target.MetaText.POR.getValue(item.Clock);
                                        int XLengthTail = (70 - (target.MetaText.POR.getValue(item.Clock + item.ID.getLength())));
                                        ...
                                        try
                                        {
                                            XLengthPrevTail = (70 - (target.MetaText.POR.getValue(item_prev.Clock + item_prev.ID.getLength())));
                                        ...
                                        try
                                        {
                                            if ((item.Clock - item_prev.Clock - item_prev.ID.getLength()) < 240)
                                            {
                                                ...
                                                float PrevY = item_prev.ID.Note - item.ID.Note + pit.getValue(PrevClock) * (float)pbs.getValue(PrevClock) / 8192.0f;
                                                int NotSameIndex = PrevClock+1;
                                                while (NotSameIndex < item_prev.Clock + item_prev.ID.getLength()-30)
                                                {
                                                    float R = item_prev.ID.Note - item.ID.Note + pit.getValue(NotSameIndex) * (float)pbs.getValue(NotSameIndex) / 8192.0f;
                                                    if (R != PrevY)
                                                    {
                                                        PrevY = R;
                                                ...
                                                float PrevItemY = pit.getValue(PrevItemClock) * (float)pbs.getValue(PrevItemClock) / 8192.0f;

                                                NotSameIndex = PrevItemClock - 1;
                                                while (NotSameIndex > item.Clock+30)
                                                {
                                                    float R = pit.getValue(NotSameIndex) * (float)pbs.getValue(NotSameIndex) / 8192.0f;
                                                    if (R != PrevItemY)
                                                    {
                                                        PrevItemY = R;
                                        ...
                                        try
                                        {
                                            if ((item_next.Clock - item.Clock - item.ID.getLength()) < 240)
                                            {
                                                ...
                                                float NextY = item_next.ID.Note - item.ID.Note + pit.getValue(NextClock) * (float)pbs.getValue(NextClock) / 8192.0f;


                                                int NotSameIndex = NextClock - 1;
                                                while (NotSameIndex > item_next.Clock+30)
                                                {
                                                    float R = item_next.ID.Note - item.ID.Note + pit.getValue(NotSameIndex) * (float)pbs.getValue(NotSameIndex) / 8192.0f;
                                                    if (R != NextY)
                                                    {
                                                        NextY = R;
                                                ...
                                                float NextItemY = pit.getValue(NextItemClock) * (float)pbs.getValue(NextItemClock) / 8192.0f;
                                        ...
                                        try
                                        {
                                            if (item.ID.LyricHandle.L0.getPhoneticSymbol() == "-" || item.ID.LyricHandle.L0.getPhoneticSymbol() == "—")
                                            {
                                                //IS - NOTE
                                                if (item_prev != null)
                                                {
                                                    int StartX = item_prev.Clock + item_prev.ID.getLength();
                                                    int EndX = item.Clock + vsq_track.MetaText.POR.getValue(item.Clock) + (int)(item.ID.getLength() * (item.ID.PMBendLength / 100.0));
                                                    int lastPit = pit.getValue(EndX);
                                                    int lastPbs = pbs.getValue(EndX);
                                                    float deltaPrev = lastPit * (float)lastPbs / 8192.0f;
                                                    float PrevNote = item_prev.ID.Note + deltaPrev;
                                                    float L = PrevNote - item.ID.Note;
                                                    LineK = cadencii.dsp.generator.UtauMath.NoteLineClass.getNoteLineMath(StartX, EndX, L, 0);
                                                }
                                            }
                                        }
                                        catch { ;}
                                        ...
                                        for (int cl = cl_start; cl < cl_end; cl++) {
                                            ...
                                            if (LineK != null)
                                            {
                                                if (LineK.isInArea(cl))
                                                {
                                                    delta = delta + (float)LineK.getPoint(cl);
                                                }
                                            }
                                            else
                                            {
                                                if (LineStart != null)
                                                {
                                                    if (LineStart.isInArea(cl - TailLen))
                                                    {
                                                        delta = (float)LineStart.getPoint(cl - TailLen);
                                                    }
                                                }
                                                if (RiseP != null)
                                                {
                                                    if (RiseP.isInArea(cl))
                                                    {
                                                        delta = (float)RiseP.getPoint(cl);
                                                    }
                                                }
                                                if (FallP != null)
                                                {
                                                    if (FallP.isInArea(cl))
                                                    {
                                                        delta = (float)FallP.getPoint(cl);
                                                    }
                                                }
                                            }

                                            if (LineEnd != null && item_next != null)
                                            {
                                                if (item_next.ID.LyricHandle.L0.getPhoneticSymbol() == "-" || item_next.ID.LyricHandle.L0.getPhoneticSymbol() == "—")
                                                {
                                                    LineEnd = null;
                                                }
                                            }
                                            ...
                                            if (LineEnd != null)
                                            {
                                                if (LineEnd.isInArea(cl))
                                                {
                                                    delta = (float)LineEnd.getPoint(cl);
                                                }
                                            }
