...
namespace cadencii.dsp.generator.STP
{
    /// <summary>
    ...
    public class WaveFile
    {
        ...
        public class DataBlock
        {
            ...
            public void ReadData(FileStream inFS, double lastSec, double SamplesPerSec)
            {
                int costsample = (int)(SamplesPerSec * lastSec / 1000.0);
                ...
                int startSample = m_NumSamples - costsample;
                inFS.Seek(startSample, SeekOrigin.Current);
                m_NumSamples = costsample;
                m_Data = new Int16[m_NumSamples];
                for (int i = 0; i < costsample; i++)
                {
                    m_Data[i] = binRead.ReadInt16();
                }
            }

            public void ReadData(FileStream inFS, double startSec, double endSec, double SamplesPerSec)
            {
                //double sec_start = processed_sample / (double)mSampleRate;

                int startsample = (int)(SamplesPerSec * startSec / 1000.0);
                int endsample = (int)(SamplesPerSec * endSec / 1000.0);
                ...
                int movveLen = startsample + 40;
                inFS.Seek(movveLen, System.IO.SeekOrigin.Begin);

                m_NumSamples = endsample - startsample + 1;
                if (m_NumSamples > (int)(m_DataSize / 2))
                {
                    m_NumSamples = (int)(m_DataSize / 2);
                };
