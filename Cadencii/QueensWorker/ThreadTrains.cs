...
namespace cadencii.QueensWorker
{
    ...
    public abstract class QueueThreadBase<T>
    {
        ...
        public QueueThreadBase(long TotalThreadCount)
        {
            List<int> MT = new List<int>();
            for (long i = 0; i < TotalThreadCount; i++)
            {
                MT.Add((int)i);
            }
            IEnumerable<T> R = (IEnumerable<T>)MT;
            m_InnerQueue = new Queue<T>(R);
            this.m_QueueCount = m_InnerQueue.Count;
        }
