...
namespace cadencii
{
    ...
    {
        private const string CONFIG_DIR_NAME = "Cadencii_UTAU";
        ...
        public static string readUtauSingerConfig(string directory, SingerConfig sc)
        {
            ...
            if (System.IO.File.Exists(character)) {
                ...
                string ENC = cadencii.utau.GetFileEncodingValue.getFileEncoding(character);
                if (ENC == "")
                {
                    ENC = cadencii.utau.GetFileEncodingValue.getFileEncoding(directory + "\\oto.ini");
                }

                if (ENC == "")
                {
                    Encodings.AddRange(AppManager.TEXT_ENCODINGS_IN_UTAU);
                }
                else
                {
                    Encodings.Add(ENC);
        ...
        public static EditedZoneUnit[] detectRenderedStatusDifference(RenderedStatus status1, RenderedStatus status2 , bool ForeceReload,double startClock,double endClock)
        {
            ...
            if (status1.config.SamplingRate != status2.config.SamplingRate ||
                 status1.config.WaveFileOutputChannel != status2.config.WaveFileOutputChannel || (ForeceReload && (startClock<=-1) && (endClock<=-1)))
            {
                zone.add(0, int.MaxValue);
            }
            if ((ForeceReload && (startClock > 0) && (endClock>1)))
            {
                zone.add(areaTempo);
                zone.add((int)startClock, (int)endClock);
            }
            else
            {
                zone.add(areaTempo);
        ...
        {
            string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Balthasar");
