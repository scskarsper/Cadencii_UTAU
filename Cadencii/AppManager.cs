...
namespace cadencii
{
    ...
    {
        public enum PreviewPlayingType
        {
            JustPlay,
            PlayAndRerenderTrack,
            PlayAndRerenderAllTrack
        }
        ...
        /// </summary>
        public static readonly string[] TEXT_ENCODINGS_IN_UTAU = new string[] { "Shift_JIS", "UTF-8", "UTF-16", "us-ascii", "GBK" };
        /// <summary>
        /// よく使うボーダー線の色
        /// </summary>
        ...
        private const string TEMPDIR_NAME = "ctau";
        ...
        /// </summary>
        private static bool previewStart(FormMain form, PreviewPlayingType ptype,double startclock,double endclock)
        {
            ...
            switch (ptype)
            {
                case PreviewPlayingType.PlayAndRerenderAllTrack:
                    for (int track = 1; track < track_count; track++)
                    {
                        mVsq.Track[track].ForceRerender = true;
                    }; break;
                case PreviewPlayingType.PlayAndRerenderTrack: mVsq.Track[selected].ForceRerender = true; 
                    mVsq.Track[selected].RenderStartClock = startclock; 
                    mVsq.Track[selected].RenderEndClock = endclock; break;
            }
            ...
            for (int track = 1; track < track_count; track++)
            {
                mVsq.Track[track].ForceRerender = false;
                mVsq.Track[track].RenderStartClock = -1;
                mVsq.Track[track].RenderEndClock = -1;
            };
                ...
                EditedZoneUnit[] areas =
                    Utility.detectRenderedStatusDifference(mLastRenderedStatus[track - 1],
                                                            new RenderedStatus(
                                                                ...
                                                                mVsq.TempoTable,
                                                                (SequenceConfig)mVsq.config.clone()), vsq_track.ForceRerender,vsq_track.RenderStartClock,vsq_track.RenderEndClock);
        ...
        public static void applyUtauParameter(VsqTrack vsq_track, VsqEvent item)
        {
            ...
            if (sc != null && mUtauVoiceDB.ContainsKey(sc.VOICEIDSTR)) {
                ...
                OtoArgs oa = db.attachFileNameFromLyric(item.ID.LyricHandle.L0.Phrase, CheckNote);
                if (item.UstEvent == null) {
        ...
        public static void setVsqCacheDir(string value)
        {
#if DEBUG
            sout.println("AppManager#setTempWaveCacheDir; before: \"" + mTempWaveDir + "\"");
            sout.println("                           after:  \"" + value + "\"");
#endif
            mVsq.cacheDir = value;
        }
        ...
        public static void setPlaying(bool value, FormMain form, double mStartClock, double mEndClock)
        {
            setPlaying(value, form, PreviewPlayingType.PlayAndRerenderTrack, mStartClock, mEndClock);
        }
        public static void setPlaying(bool value, FormMain form, PreviewPlayingType ptype)
        {
            setPlaying(value, form, ptype, -1, -1);
        }
        public static void setPlaying(bool value, FormMain form, PreviewPlayingType ptype, double mStartClock, double mEndClock)
        {
#if DEBUG
            ...
            lock (mLockerPlayingProperty) {
                ...
                if (previous != mPlaying) {
                    if (mPlaying) {
                        try {
                            if (previewStart(form,ptype,mStartClock,mEndClock)) {
#if DEBUG
                ...
                if (editorConfig.UseProjectCache) {
                    ...
                    if (!currentCacheDir.Equals(cacheDir)) {
                        for (int i = 1; i < mVsq.Track.Count; i++) {
                            try
                            {
                                string newCacheDir = Path.Combine(cacheDir, "TrackCache_" + i.ToString());
                                string curCacheDir = Path.Combine(currentCacheDir, "TrackCache_" + i.ToString());
        ...
        {
            Cadencii.splash.setStatus("系统正在初始化");
            loadConfig();
            ...
            Cadencii.splash.setStatus("开始初始化歌手形象");
            ...
            */
            ...
            Cadencii.splash.setStatus("系统初始化结束");
        }
        ...
        {   
            try
            {
                Cadencii.splash.setStatus("正在重建声库缓存");
            }
            catch { ;}
            try
            {
                mMainWindow.setPrograssBarVisible(true);
                mMainWindow.setPrograssBarName("正在重建声库缓存");
                mMainWindow.setPrograssBarMaxium(editorConfig.UtauSingers.Count);
            }
            catch { ;}
            mUtauVoiceDB.Clear();
            int ak = 0;
            foreach (var config in editorConfig.UtauSingers) {
                ...
                try
                {
                    Cadencii.splash.setStatus("正在重建声库缓存 (" + ak.ToString() + "/" + editorConfig.UtauSingers.Count + ")");
                }
                catch { ;}
                try
                {
                    mMainWindow.setPrograssBarValue(ak);
        ...
        {
            Cadencii.splash.setStatus("开始加载配置设置");
            ...
            if (appdata.Equals("")) {
                ...
                editorConfig = initEditConfig(editorConfig);
                return;
            }
            ...
            ret = initEditConfig(ret);

            editorConfig = ret;

            keyWidth = editorConfig.KeyWidth;
        }
        private static EditorConfig initEditConfig(EditorConfig cfg)
        {
            EditorConfig ret = cfg;
            Cadencii.splash.setStatus("开始检索本地渲染引擎");
            //LoadEngine
            #region
            string localResamplerFolor = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + "\\engines";
            string localWavtool = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + "\\wavtool.exe";
            while (!System.IO.File.Exists(localWavtool))
            {
                OpenFileDialog OFD = new OpenFileDialog();
                OFD.Filter = "wavtool.exe|*.exe";
                OFD.Title = "Please input the UTAU-WAV engine WAVTOOL.exe";
                OFD.ShowDialog();
                string cc = OFD.FileName;
                if (System.IO.File.Exists(cc))
                {
                    System.IO.File.Copy(cc, localWavtool);
                }
            }
            if (System.IO.File.Exists(localWavtool))
            {
                ret.PathWavtool = localWavtool;
                ret.WavtoolWithWine = false;
            }
            System.IO.DirectoryInfo DI=new DirectoryInfo(localResamplerFolor);
            if(!DI.Exists)
            {
                DI.Create();
            }
            ret.clearResampler();
            if (System.IO.File.Exists(localResamplerFolor + "\\resampler.exe"))
            {
                ret.addResampler(localResamplerFolor + "\\resampler.exe", false);
            }
            foreach (FileInfo F in DI.GetFiles("*.exe"))
            {
                if (F.Name.ToLower() != "resampler.exe")
                {
                    ret.addResampler(F.FullName, false);
                }
            }
            #endregion

            //LoadVoiceDB
            Cadencii.splash.setStatus("开始检索本地声库文件");
            #region
            string VdbRoot = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + "\\voicedb";
            if (!System.IO.Directory.Exists(VdbRoot))
            {
                try
                {
                    System.IO.Directory.CreateDirectory(VdbRoot);
                }
                catch { ;}
            }
            List<string> OtoRoot=FindOtoINI(new DirectoryInfo(VdbRoot));

            for(int k=0;k<cfg.UtauSingers.Count;k++)
            {
                string SC = cfg.UtauSingers[k].VOICEIDSTR;
                while (OtoRoot.Contains(SC))
                {
                    try
                    {
                        OtoRoot.Remove(SC);
                ...
                try
                {
                    Cadencii.splash.setStatus("开始初始化本地声库("+(k+1).ToString()+"/"+cfg.UtauSingers.Count+")");
                    Utility.readUtauSingerConfig(SC, cfg.UtauSingers[k]);
                }
                catch { ;}
            }
            int ak = 0;
            foreach (string Oto in OtoRoot)
            {
                ak++;
                SingerConfig sc = new SingerConfig();
                Cadencii.splash.setStatus("开始创建本地新声库索引(" + (ak).ToString() + "/" + OtoRoot.Count + ")");
                Utility.readUtauSingerConfig(Oto, sc);
                sc.Program = cfg.UtauSingers.Count;
                cfg.UtauSingers.Add(sc);
            }
            #endregion
            Cadencii.splash.setStatus("开始初始化拆音词书");
            UtauSplitDictionary.InitDictionaryLibrary();
            return ret;
        }

        public static utausplitdictionary.DictionaryManager UtauSplitDictionary = new utausplitdictionary.DictionaryManager();

        private static List<string> FindOtoINI(DirectoryInfo Root)
        {
            List<string> Ret = new List<string>();
            if (Root.Exists)
            {
                if (File.Exists(Path.Combine(Root.FullName, "oto.ini")) && File.Exists(Path.Combine(Root.FullName, "character.txt")))
                {
                    Ret.Add(Root.FullName);
                }
                else
                {
                    foreach (DirectoryInfo DRoot in Root.GetDirectories())
                    {
                        Ret.AddRange(FindOtoINI(DRoot));
                    }
                }
            }
            return Ret;
        ...
        public static EditorConfig initEditConfig_Auto(EditorConfig cfg)
        {
            EditorConfig ret = cfg;
            string localResampler = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + "\\resampler.exe";
            string localWavtool = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + "\\wavtool.exe";
            string wavT = ret.PathWavtool;
            FileInfo lorf=new FileInfo(localResampler);
            if (!System.IO.File.Exists(wavT))
            {
                if(System.IO.File.Exists(localWavtool))
                {
                    ret.PathWavtool = localWavtool;
            ...
            bool haveR=false;
            int i=0;
            while (i < ret.getResamplerCount())
            {
                string r = ret.getResamplerAt(i);
                FileInfo rf=new FileInfo(r);
                if (localResampler==r)
                {
                    haveR = true;
                }
            ...
            if (!haveR)
            {
                if (lorf.Exists)
                {
                    ret.addResampler(localResampler, false);
                }
            }
            return ret;
        }
