...
namespace cadencii
{
    ...
    public partial class FormMain : Form, FormMainUi, PropertyWindowListener
    {
        ...
        public const string _APP_NAME = "Cadencii_UTAU";
        /// <summary>
        /// 表情線の先頭部分のピクセル幅
        /// </summary>
                ...
                if (System.IO.File.Exists(file)) {
                    ...
                    if (low_file.EndsWith(".xvsq")) {
                        ...
                        try {
                            ...
                            vsq.cacheDir = AppManager.getCacheDir(file);
...
/*                    RendererKind kind = VsqFileEx.getTrackRendererKind(vsq.Track[selected]);
        ...
        public List<ValuePairOfStringArrayOfKeys> getDefaultShortcutKeys()
        {
            ...
            List<ValuePairOfStringArrayOfKeys> ret = new List<ValuePairOfStringArrayOfKeys>(new ValuePairOfStringArrayOfKeys[]{
                ...
                new ValuePairOfStringArrayOfKeys( menuTrackRenderer.Name, new Keys[]{} ),
        ...
        public void updateRendererMenu()
        {
            ...
            // サブアイテムの個数を整える
            int delta = count - menuTrackRenderer.DropDownItems.Count;
            if (delta > 0) {
                // 増やす
                for (int i = 0; i < delta; i++) {
                    cMenuTrackTabRenderer.DropDownItems.Add("", null, new EventHandler(handleChangeRenderer));
                    //menuTrackRenderer.DropDownItems.Add("", null, new EventHandler(handleChangeRenderer));
                    menuTrackRenderer.DropDownItems.Add("", null, new EventHandler(handleChangeRenderer));
                }
            } else if (delta < 0) {
                // 減らす
                for (int i = 0; i < -delta; i++) {
                    cMenuTrackTabRenderer.DropDownItems.RemoveAt(0);
                    menuTrackRenderer.DropDownItems.RemoveAt(0);
                }
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                menuTrackRenderer.DropDownItems[i].Text = name;
                cMenuTrackTabRenderer.DropDownItems[i].Text = name;

                menuTrackRenderer.DropDownItems[i].ToolTipText = path;
                cMenuTrackTabRenderer.DropDownItems[i].ToolTipText = path;
            }
        }
        ...
        public void clearExistingData()
        {
            ...
            if (AppManager.isPlaying()) {
                AppManager.setPlaying(false, this,AppManager.PreviewPlayingType.JustPlay);
            }
            waveView.unloadAll();
        }
        ...
        public void executeLyricChangeCommand()
        {
            ...
            if ((phonetic_symbol_edit_mode && AppManager.mInputTextBox.Text != original_symbol[0]) ||
                ...
                if (phonetic_symbol_edit_mode) {
                    ...
                    if (!symbol_protected[0]) {
                        ...
                        SymbolTableEntry entry = UtauSymbolTable.UtauSymbolTable.attatch(phrase,0,"Default");//SymbolTable.attatch(phrase[0]);
                        if (entry == null) {
            ...
            if (vsq_track.SplitDictionaryName != "Default")
            {
                int LID = vsq_track.getEventIndex(last_selected_event.original.InternalID) - 1;
                TransformSelectedPhoneme(vsq_track.SplitDictionaryName, LID<0?0:LID,3);
            }
        }
        ...
        public void processSpecialShortcutKey(KeyEventArgs e, bool onPreviewKeyDown)
        {
            ...
            if (!onPreviewKeyDown && flipPlaying) {
                if (AppManager.isPlaying()) {
                    ...
                    AppManager.setPlaying(true, this,AppManager.PreviewPlayingType.JustPlay);
                }
            }
            if (e.KeyCode == System.Windows.Forms.Keys.Tab) {
                focusPianoRoll();
        ...
        public void forward()
        {
            ...
            if (cl_new <= hScroll.Maximum + (pictPianoRoll.getWidth() - AppManager.keyWidth) * controller.getScaleXInv()) {
                ...
                ensureCursorVisible();
                AppManager.setPlaying(playing, this,AppManager.PreviewPlayingType.JustPlay);
                refreshScreen();
            }
        }
        ...
        public void applyLanguage()
        {
            ...
            if (AppManager.isPlaying()) {
                ...
                stripBtnPlay_Rerender.Text = _("Stop");
                stripBtnPlay_RerenderAll.Text = _("Stop");
            } else {
                stripBtnPlay.Text = _("Play");
                stripBtnPlay_Rerender.Text = _("PlayAndRender");
                stripBtnPlay_RerenderAll.Text = _("PlayAndRenderAll");
            }
            ...
            menuTrackRenderer.Mnemonic(Keys.D4);
            ...
            phonemeTransformationInSelectionToolStripMenuItem.Text = _("Selection Phoneme transformation");
            phonemeTransformationInSelectionToolStripMenuItem.Mnemonic(Keys.G);
            phonemeDictionaryToolStripMenuItem.Text = _("Phoneme Dictionary");
            phonemeDictionaryToolStripMenuItem.Mnemonic(Keys.C);
            clearCacheFilesToolStripMenuItem.Text = _("Clear Cache Files");
            ...
            symbolToolStripMenuItem.Text = _("PhonemeSymbol");
            symbolToolStripMenuItem.Mnemonic(Keys.P);
        ...
        public void importLyric()
        {
            ...
            try {
                ...
                if (dr == DialogResult.OK) {
                    ...
                    for (int i = 0; i < phrases.Length; i++) {
                        ChangedLength++;
                        SymbolTableEntry entry = UtauSymbolTable.UtauSymbolTable.attatch(phrases, i,"Default"); //SymbolTable.attatch(phrases[i]);
            ...
            if (vsq_track.SplitDictionaryName != "Default")
            {
                TransformSelectedPhoneme(vsq_track.SplitDictionaryName, ChangedLength);
            }
        }
...
#endif // ENABLE_SCRIPT
 {
                        ...
                        foreach (var item in vsq_track.getNoteEventIterator()) {
                            ...
                            if (0 <= program && program < AppManager.editorConfig.UtauSingers.Count) {
                                ...
                                if (AppManager.mUtauVoiceDB.ContainsKey(sc.VOICEIDSTR))
                                {
                                    ...
                                    {
                                        CheckNote = item.UstEvent.ReplaceNoteID>0?item.UstEvent.ReplaceNoteID:note;
                                    }
                                    is_valid_for_utau = cadencii.UtauSymbolTable.UtauSymbolTable.CheckSymbolExists(lyric_en,db, CheckNote);
                                    /*
                                    OtoArgs oa = db.attachFileNameFromLyric(lyric_jp, note);
                                    if (oa.fileName == null ||
                                        (oa.fileName != null && oa.fileName == "")) {
                                            is_valid_for_utau = true;// false;
                                    } else {
        ...
        public bool openVsqCor(string file)
        {
            ...
            // キャッシュwaveなどの処理
            if (true || AppManager.editorConfig.UseProjectCache) {
                ...
                string estimatedCacheDir = Path.Combine(dir, name + ".ctau"); // ファイル名から推測されるキャッシュディレクトリ
                if (cacheDir == null) {
                    cacheDir = "";
                }
                if (cacheDir != "" &&
                    ...
                    for (int i = 1; i < vsq.Track.Count; i++) {
                        ...
                        string newCacheDir = Path.Combine(estimatedCacheDir, "TrackCache_" + i.ToString());
                        string curCacheDir = Path.Combine(cacheDir, "TrackCache_" + i.ToString());
                        DirectoryInfo CacheChild = new DirectoryInfo(curCacheDir);
                        foreach (FileInfo F in CacheChild.GetFiles())
                        {
                            try
                            {
                                F.MoveTo(Path.Combine(newCacheDir, F.Name));
                            }
                            catch { ;}
                        }
                ...
                AppManager.setVsqCacheDir(cacheDir);
                #endregion
            }
            return false;
        }
        ...
        public void registerEventHandlers()
        {
            ...
            menuTrackRenderer.MouseEnter += new EventHandler(handleMenuMouseEnter);
            //UTAUはresamplerを識別するのでmenuTrackRendererのサブアイテムのClickイベントを拾う
            //menuTrackRenderer.Click += new EventHandler( handleChangeRenderer );
        ...
        public void AppManager_PreviewAborted(Object sender, EventArgs e)
        {
            ...
            stripBtnPlay.Text = _("Play");
            stripBtnPlay_Rerender.ImageKey = "control.png";
            stripBtnPlay_RerenderAll.ImageKey = "control.png";
            stripBtnPlay_Rerender.Text = _("PlayAndRender");
            stripBtnPlay_RerenderAll.Text = _("PlayAndRenderAll");
            timer.Stop();
        ...
        public void AppManager_PreviewStarted(Object sender, EventArgs e)
        {
            ...
            stripBtnPlay.Text = _("Stop");
            stripBtnPlay_Rerender.ImageKey = "control_pause.png";
            stripBtnPlay_Rerender.Text = _("Stop");
            stripBtnPlay_RerenderAll.ImageKey = "control_pause.png";
            stripBtnPlay_RerenderAll.Text = _("Stop");
        }
...

 {
        ...
        public void clearPianoMouseEvent()
        {
            if (mMouseDowned)
            {
                pictPianoRoll_MouseUp(null, null);
            }
        }
            ...
            try {
                ...
                if (mGameMode == GameControlMode.NORMAL) {
                    ...
                    if (!event_processed && !btn_o && mLastBtnO) {
                        ...
                        AppManager.setPlaying(!AppManager.isPlaying(), this, AppManager.PreviewPlayingType.JustPlay);
        ...
        public void menuFileOpenUst_Click(Object sender, EventArgs e)
        {
            ...
            try {
                ...
                clearExistingData();
                vsq.cacheDir = AppManager.getCacheDir(filename);
                AppManager.setVsqFile(vsq);
                setEdited(true);
                AppManager.mMixerWindow.updateStatus();
                clearTempWave();
        ...
        public void menuFileOpenVsq_Click(Object sender, EventArgs e)
        {
            ...
            try {
                ...
                VsqFileEx vsq = new VsqFileEx(actualReadFile, "UTF-8");
                if (isVsqx) {
        ...
        public void menuSettingPreference_Click(Object sender, EventArgs e)
        {
            try {
                ...
                if (dr == DialogResult.OK) {
                    ...
                    reloadMidiIn();
#endif
                    if (mDialogPreference.isChangedUtauSingers())
                    {
                    ...
                    if (AppManager.editorConfig.UseProjectCache && !mDialogPreference.isUseProjectCache()) {
                        ...
                        if (file != null && !file.Equals("")) {
                            string dir = PortUtil.getDirectoryName(file);
                            string name = PortUtil.getFileNameWithoutExtension(file);
                            string projectCacheDir = Path.Combine(dir, name + ".ctau");
                            AppManager.getVsqFile().cacheDir = projectCacheDir;
                            AppManager.setTempWaveDir(projectCacheDir);
                    ...
                    AppManager.editorConfig.UseProjectCache = true;// mDialogPreference.isUseProjectCache();
                    AppManager.editorConfig.DoNotUseVocaloid1 = true;// !mDialogPreference.isVocaloid1Required();
                    AppManager.editorConfig.DoNotUseVocaloid2 = true;// !mDialogPreference.isVocaloid2Required();
                    AppManager.editorConfig.BufferSizeMilliSeconds = mDialogPreference.getBufferSize();
                    AppManager.editorConfig.DefaultSynthesizer = RendererKind.UTAU;//mDialogPreference.getDefaultSynthesizer();
                    AppManager.editorConfig.UseUserDefinedAutoVibratoType = mDialogPreference.isUseUserDefinedAutoVibratoType();
                    AppManager.editorConfig.UseWideCharacterWorkaround = false;// mDialogPreference.isEnableWideCharacterWorkaround();
        ...
        void TransformAllPhoneme(string DictionaryName)
        {
            List<int> internal_ids = new List<int>();
            ...
            //vsq_track.SplitDictionaryName = DictionaryName;
            List<VsqEvent> trackevents = new List<VsqEvent>();
            foreach (var item in vsq_track.getNoteEventIterator())
            {
                trackevents.Add(item);
            }
            if (trackevents.Count > 1) setPrograssBarVisible(true);
            setPrograssBarName("歌词修改");
            setPrograssBarMaxium(trackevents.Count);
            for (int k = 0; k < trackevents.Count; k++)
            {
                setPrograssBarValue(k);
                ...
                phrases[1] = phrase;
                SymbolTableEntry entry = UtauSymbolTable.UtauSymbolTable.attatch(phrases, 1, DictionaryName);//SymbolTable.attatch(phrases,1);
                if (entry == null)
                {
                    continue;
                }
                ...
                ids.Add(idNew);
                internal_ids.Add(trackevents[k].InternalID);
                string SymbolOldA = symbolOld;
                string SymbolA = symbolResult;
                if (SymbolOldA.IndexOf(",") > 0) { SymbolOldA = SymbolOldA.Split(',')[0]; };
                if (SymbolA.IndexOf(",") > 0) { SymbolA = SymbolA.Split(',')[0]; };
                if (SymbolA != SymbolOldA)
                {
                    try
                    {
                        VsqEvent singer = vsq_track.getSingerEventAt(trackevents[k].Clock);
                        ...
                        {
                            UtauVoiceDB db = AppManager.mUtauVoiceDB[sc.VOICEIDSTR];
                            int CheckNote = id.Note;
                            if (trackevents[k].UstEvent != null)
                            {
                                CheckNote = trackevents[k].UstEvent.ReplaceNoteID > 0 ? trackevents[k].UstEvent.ReplaceNoteID : id.Note;
                            }
                            OtoArgs oa = db.attachFileNameFromLyric(SymbolA, CheckNote);
                            if (trackevents[k].UstEvent == null)
                            {
                                trackevents[k].UstEvent = new UstEvent();
                            }
                            trackevents[k].UstEvent.setVoiceOverlap(oa.msOverlap);
                            trackevents[k].UstEvent.setPreUtterance(oa.msPreUtterance);
                        }
                    }
                    catch { ;}
                }
            }
            setPrograssBarVisible(false);
            if (ids.Count <= 0)
            {
                return;
            }
        ...
        void TransformSelectedPhoneme(string DictionaryName)
        {
            int a,b;
            VsqEvent start,end;
            getSelections(out a, out b, out start, out end);
            ...
            int StartIndex = vsq_track.getEventIndex(start.InternalID);
            int EndIndex = vsq_track.getEventIndex(end.InternalID);
            for(int i=StartIndex-1;i>=0;i--)
            {
                try
                {
                    if (vsq_track.getEventByIndex(i).ID.LyricHandle.L0.PhoneticSymbol != "-" && vsq_track.getEventByIndex(i).ID.LyricHandle.L0.PhoneticSymbol != "—")
                    {
                        StartIndex = i;
                        break;
                    }
            ...
            for (int i = EndIndex + 1; i < vsq_track.getEventList().Events.Count; i++)
            {
                try
                {
                    if (vsq_track.getEventByIndex(i).ID.LyricHandle.L0.PhoneticSymbol != "-" && vsq_track.getEventByIndex(i).ID.LyricHandle.L0.PhoneticSymbol != "—")
            ...
            TransformSelectedPhoneme_Kernel(DictionaryName, StartIndex,EndIndex-StartIndex+1);
        }
        void TransformSelectedPhoneme(string DictionaryName,int SelectLength)
        {
            int a, b;
            VsqEvent start, end;
            getSelections(out a, out b, out start, out end);
            ...
            VsqTrack vsq_track = vsq.Track[AppManager.getSelected()]; 
            int StartIndex = vsq_track.getEventIndex(start.InternalID);
            int EndIndex = StartIndex + SelectLength;
            for (int i = StartIndex - 1; i >= 0; i--)
            {
                try
                {
        ...
        void TransformSelectedPhoneme(string DictionaryName, int SelectedStartindex, int SelectLength)
        {
            VsqFileEx vsq = AppManager.getVsqFile();
            if (vsq == null)
            {
            ...
            VsqTrack vsq_track = vsq.Track[AppManager.getSelected()];
            int StartIndex = SelectedStartindex;
            int EndIndex = StartIndex + SelectLength;
            for (int i = StartIndex - 1; i >= 0; i--)
            {
                try
        ...
        void TransformSelectedPhoneme_Kernel(string DictionaryName,int SelectedStartindex,int SelectLength)
        {
            try
            {
                List<int> internal_ids = new List<int>();
                ...
                int selectedcount = SelectLength;
                if (SelectLength <= 0)
                {
                    selectedcount = AppManager.itemSelection.getEventCount();
                }
                if (selectedcount > 1) setPrograssBarVisible(true);
                setPrograssBarName("歌词修改");
                setPrograssBarMaxium(selectedcount);
                setPrograssBarValue(0);
                List<VsqEvent> trackevents = new List<VsqEvent>();
                bool isStart = false;
                int c = 0;
                for (int i = SelectedStartindex; i < SelectedStartindex + SelectLength; i++)
                {
                    trackevents.Add(vsq_track.getEventByIndex(i));
                }
                setPrograssBarMaxium(trackevents.Count);
                for (int k = 0; k < trackevents.Count; k++)
                {
            ...
            {
                setPrograssBarVisible(false);
                MessageBox.Show("请选择要修改的音符区域！");
            }
        }


        void ReloadSelectedOtoParamers()
        {
            try
            {
                int a, b;
                ...
                int selected = AppManager.getSelected();
                VsqTrack vsq_track = vsq.Track[selected];
                int SelectedStartindex = vsq_track.getEventIndex(start.InternalID);
                int selectedcount = vsq_track.getEventIndex(end.InternalID) - SelectedStartindex + 1;
                if (selectedcount > 1) setPrograssBarVisible(true);
                setPrograssBarName("重新加载Oto属性");
                setPrograssBarMaxium(selectedcount);
                setPrograssBarValue(0);
                List<VsqEvent> trackevents = new List<VsqEvent>();
                int c = 0;

                for (int i = SelectedStartindex; i <= vsq_track.getEventIndex(end.InternalID); i++)
                {
                    trackevents.Add(vsq_track.getEventByIndex(i));
                }
                ...
                for (int k = 0; k < trackevents.Count; k++)
                {
                    ...
                    Application.DoEvents();
                    VsqID id = trackevents[k].ID;
                    string SymbolA = trackevents[k].ID.LyricHandle.L0.getPhoneticSymbol();
                    if (SymbolA == "-" || SymbolA == "—")
                    {
                        continue;
                    }
        ...
        {
            //TransformAllPhoneme(vsq_track.SplitDictionaryName);
        }
        ...
        public void getSelections(out int StartClock, out int EndClock, out VsqEvent StartItem, out VsqEvent EndItem)
        {
            int SelectStart = -1;
            int SelectEnd = -1;
            StartItem = null;
            ...
            try
            {
                ...
                int selected = AppManager.getSelected();
                VsqTrack vsq_track = vsq.Track[selected];
                List<SelectedEventEntry> SelectionArea=AppManager.itemSelection.getSelectedEvents();
                int startItemID = vsq_track.getEventList().findIndexFromID(SelectionArea[SelectionArea.Count - 1].original.InternalID);
                int endItemID = vsq_track.getEventList().findIndexFromID(SelectionArea[0].original.InternalID);
                ...
                try
                {
                    while (vsq_track.getEventByIndex(endItemID + 1).ID.LyricHandle.L0.PhoneticSymbol == "-" || vsq_track.getEventByIndex(endItemID + 1).ID.LyricHandle.L0.PhoneticSymbol == "—")
                    {
                        endItemID++;
                    }
                }
                ...
                try
                {
                    while (vsq_track.getEventByIndex(startItemID).ID.LyricHandle.L0.PhoneticSymbol == "-" || vsq_track.getEventByIndex(startItemID).ID.LyricHandle.L0.PhoneticSymbol == "—")
                    {
                        startItemID--;
                    }
                }
                catch { ;}
                if (startItemID <= 0) startItemID = 1;
                int selectedcount = endItemID - startItemID + 1;
                if (selectedcount > 0)
                {
                    SelectStart = vsq_track.getEventByIndex(startItemID).Clock;
                    StartItem = vsq_track.getEventByIndex(startItemID);
                    if (endItemID < startItemID)
                    {
                        SelectEnd = int.MaxValue;
                        EndItem=vsq_track.getEventByIndex(vsq_track.getEventList().Events.Count-1);
                    }
                    else
                    {
                        SelectEnd = vsq_track.getEventByIndex(endItemID).Clock;
                        EndItem = vsq_track.getEventByIndex(endItemID);
                        if (SelectEnd == SelectStart)
                        {
                            SelectEnd = vsq_track.getEventByIndex(startItemID).Clock + vsq_track.getEventByIndex(startItemID).ID.getLength();
                        }
                    }
                }
            }
            catch { ;}
            StartClock = SelectStart;
            EndClock = SelectEnd;
        }

        public void stripBtnPlay_This_Click(Object sender, EventArgs e)
        {
            //GetSelected
            int SelectStart = -1;
            int SelectEnd=-1;
            VsqEvent a,b;
            getSelections(out SelectStart, out SelectEnd, out a, out b);
            //ThisIs A Bug START
            /*if (SelectStart < 1920 + 1920 && SelectStart>0)
            {
                SelectStart = 0;
            }*/
            //ThisIs A Bug END
            AppManager.setPlaying(!AppManager.isPlaying(), this, AppManager.PreviewPlayingType.PlayAndRerenderTrack, SelectStart, SelectEnd);
            focusPianoRoll();
        }

        public void stripBtnPlay_All_Click(Object sender, EventArgs e)
        {
            AppManager.setPlaying(!AppManager.isPlaying(), this, AppManager.PreviewPlayingType.PlayAndRerenderAllTrack);
            focusPianoRoll();
        }
        ...
        void toolBarPosition_ButtonClick(Object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
        {
            ...
            else if (e.Button == stripBtnPlay_RerenderAll)
            {
                stripBtnPlay_All_Click(e.Button, new EventArgs());
            }
            else if (e.Button == stripBtnPlay_Rerender)
            {
                stripBtnPlay_This_Click(e.Button, new EventArgs());
 ...
 sender == menuSettingPositionQuantizeOff) {
        ...
        public void handleChangeRenderer(Object sender, EventArgs e)
        {
            ...
            if (menu_handler != null && menu_handler.RendererKind != RendererKind.UTAU) {
                ...
                // イベント送信元のアイテムが，cMenuTrackTabRendererまたは
                // menuTrackRendererUTAUのサブアイテムかどうかをチェック
                if (sender is System.Windows.Forms.ToolStripMenuItem) {
                    System.Windows.Forms.ToolStripMenuItem item = (System.Windows.Forms.ToolStripMenuItem)sender;
                    resampler_index = cMenuTrackTabRenderer.DropDownItems.IndexOf(item);
                    if (resampler_index < 0) {
                        resampler_index = menuTrackRenderer.DropDownItems.IndexOf(item);
                    }
                }
                if (resampler_index < 0) {
                    // 検出できないのでbailout
            ...
            renderer_menu_handler_.ForEach((handler) => handler.updateCheckedState(kind));
            for (int i = 0; i < cMenuTrackTabRenderer.DropDownItems.Count; i++) {
                ((System.Windows.Forms.ToolStripMenuItem)cMenuTrackTabRenderer.DropDownItems[i]).Checked = (i == resampler_index);
            }
            for (int i = 0; i < menuTrackRenderer.DropDownItems.Count; i++) {
                ((System.Windows.Forms.ToolStripMenuItem)menuTrackRenderer.DropDownItems[i]).Checked = (i == resampler_index);
            }
            setEdited(true);
            refreshScreen();
        }
        ...
        ToolStripMenuItem CreateDictionarySelector(string DictionaryName)
        {
            ToolStripMenuItem ret = new ToolStripMenuItem();
            ret.Name = "TransDic_" + DictionaryName;
            ret.Size = new System.Drawing.Size(152, 22);
            ret.Text = DictionaryName;
            ret.Tag = DictionaryName;
            ret.Click += new EventHandler(ChangePhonemeDictionary);
            return ret;
        }
        private void ChangePhonemeDictionary(object sender, EventArgs args)
        {
            VsqFileEx vsq = AppManager.getVsqFile();
            if (vsq == null)
            {
            ...
            int selected = AppManager.getSelected();
            VsqTrack vsq_track = vsq.Track[selected];
            string oldT = vsq_track.SplitDictionaryName;
            vsq_track.SplitDictionaryName = (string)((ToolStripMenuItem)sender).Tag;
            if (oldT != vsq_track.SplitDictionaryName)
            {
                TransformAllPhoneme(vsq_track.SplitDictionaryName);
            }
        }
        private void phonemeDictionaryToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }
        void LoadUTAUDictionary()
        {
            foreach (string s in AppManager.UtauSplitDictionary.getDictionaryNames())
            {
                phonemeDictionaryToolStripMenuItem.DropDownItems.Add(CreateDictionarySelector(s));
            }
        }

        private void aboutCadenciiUTAUToolStripMenuItem_Click(object sender, EventArgs e)
        {
            StringBuilder SB = new StringBuilder();
            SB.AppendLine("关于Cadencii_UTAU");
            SB.AppendLine("");
            SB.AppendLine("Cadencii_UTAU分支精简和修改自Cadencii 3.5.4\r\n重写内容主要包含：");
            SB.AppendLine("\tUTAU引擎适应性修改");
            SB.AppendLine("\tPOR自动滑音运算");
            SB.AppendLine("\t引擎和声库自动加载");
            SB.AppendLine("\t自动拆音模块");
            SB.AppendLine("\t声库描述文件编码集识别（防止乱码）");
            SB.AppendLine("\t其余BUG若干");
            SB.AppendLine("");
            SB.AppendLine("由于对配置文件和格式文件添加了额外的描述变量，为防止与Cadencii其他分支出线配置和代码污染\t本工程于GitHub创建额外的开源分支，协议沿袭源Cadencii");
            SB.AppendLine("");
            SB.AppendLine("Powered By 冥月星璇（http://twitter.com/scskarsper,http://weibo.com/scskarsper）");
            MessageBox.Show(SB.ToString());
        }

        private void clearCacheFilesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string P = AppManager.getVsqFile().cacheDir;
            try
            {
                DirectoryInfo PD = new DirectoryInfo(P);
                foreach (DirectoryInfo D in PD.GetDirectories("TrackCache_*"))
                {
                    try
                    {
                        D.Delete(true);
            ...
            cadencii.UtauWaveGenerator.ClearCacheList();
        }

        private void menuLyricPhonemeTransformation_Click_1(object sender, EventArgs e)
        {
            VsqFileEx vsq = AppManager.getVsqFile();
            if (vsq == null)
            {
        ...
        private void phonemeTransformationInSelectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            VsqFileEx vsq = AppManager.getVsqFile();
            if (vsq == null)
            {
            ...
            int selected = AppManager.getSelected();
            VsqTrack vsq_track = vsq.Track[selected];
            TransformSelectedPhoneme(vsq_track.SplitDictionaryName);
        }

        public void setPrograssBarVisible(bool value)
        {
            this.Invoke(new MethodInvoker(() =>
            {
                clearPianoMouseEvent();
                mLockMouse = value;
                workLab.Visible = value;
                WorkProgress.Visible = value;
                WorkSeparator.Visible = value;
                WorkName.Visible = value;
                workLab.Text = "0/0";
                WorkName.Text = "进度：";
            }));
        }
        public void setPrograssBarName(string Name)
        {
            this.Invoke(new MethodInvoker(() =>
            {
                WorkName.Text = Name + "：";
        ...
        public void setPrograssBarMaxium(int Value)
        {
            this.Invoke(new MethodInvoker(() =>
            {
                WorkProgress.Maximum = Value;
            }));
        }
        public void setPrograssBarValue(int Value)
        {
            this.Invoke(new MethodInvoker(() =>
            {
                if (Value > WorkProgress.Maximum)
                {
                    Value = WorkProgress.Maximum;
                }
                WorkProgress.Value = Value;
                workLab.Text = Value + "/" + WorkProgress.Maximum.ToString();
            }));
        }

        private void workLab_Click(object sender, EventArgs e)
        {
        ...
        private void reloadOtoParamersToolStripMenuItem_Click(object sender, EventArgs e)
        {
            VsqFileEx vsq = AppManager.getVsqFile();
            if (vsq == null)
            {
            ...
            int selected = AppManager.getSelected();
            VsqTrack vsq_track = vsq.Track[selected];
            ReloadAllOtoParamers();
        }

        private void reloadOtoParamersInSelectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            VsqFileEx vsq = AppManager.getVsqFile();
            if (vsq == null)
            {
