        ...
        public virtual void adjustClockToMatchWith(double tempo)
        {
            ...
            for (int track = 1; track < numTrack; track++) {
                ...
                for (Iterator<VsqEvent> itr = vsq_track.getEventIterator(); itr.hasNext(); ) {
                    ...
                    //SPLIT======只能提前切分了=======
                    if (item.UstEvent.SplitisPercent)
                    {
                        item.UstEvent.SplitLength = (int)(item.ID.getLength() * (item.UstEvent.SplitLength / 100.0));
                        if (item.UstEvent.SplitLength > item.ID.getLength())
                        {
                            item.UstEvent.SplitLength = 0;
                        }
                        item.UstEvent.SplitisPercent = false;
                    }
                    double sec_splitline = getSecFromClock(clock + item.ID.getLength() - item.UstEvent.SplitLength);
                    int clock_splitline = (int)(sec_splitline * 8.0 * tempo);
                    item.UstEvent.SplitLength = clock_end - clock_splitline;
                    //SPLIT
